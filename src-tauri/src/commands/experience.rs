...
#[tauri::command]
pub fn get_experiences(state: tauri::State<AppDatabase>) -> Result<Vec<experience::Experience>, String> {
    info!("get_experiences called");
    let conn = state.conn.lock().unwrap();
    let result = experience::get_experiences(&conn).map_err(|err| {
...
pub fn insert_experience(
    ...
    let conn = state.conn.lock().unwrap();
    match experience::insert_experience(&conn, experience) {
        Ok(_) => {
...
pub fn update_experience(
    ...
    let conn = state.conn.lock().unwrap();
    match experience::update_experience(&conn, experience, exp_id) {
        Ok(_) => {
...
pub fn delete_experience(
    ...
    let conn = state.conn.lock().unwrap();
    match experience::delete_experience(&conn, exp_id) {
        Ok(_) => {
